...
class LLM:
    ...
    def _get_config(self):
        ...
        return (
            config["LLM"]["HOST"],
            config["LLM"]["API_KEY"],
            config["LLM"]["API_KEY_PRIMARY_VAL"],
        )
    ...
    def resume_summary(self, text):
        ...
        preset_text = [
            {"role": "system", "content": str(self.resume_prompt)},
    ...
    def recruit_summary(self, text):
        ...
        preset_text = [
            {"role": "system", "content": str(self.recruit_prompt)},
            {"role": "user", "content": str(text)},
        ]
        request_data = {
            "messages": preset_text,
    ...
    def make_question(self, resume_summary, recruit_summary):
        ...
        user_input = f"'이력서'\n{resume_summary}'채용공고'\n{recruit_summary}\n\n- 질문생성\n\n[직무 역량 검증]\n\n[경험 검증]"
        preset_text = [
            {"role": "system", "content": str(self.question_prompt)},
    ...
    def interviewer_feel(self, text):
        ...
        user_input = f"#답변\n{text}"
        preset_text = [
            {"role": "system", "content": str(self.interviewer_feel_prompt)},
            {"role": "user", "content": user_input},
        ]
        request_data = {
            "messages": preset_text,
